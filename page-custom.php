							...
							<article id="post-<?php the_ID(); ?>" <?php post_class( 'cf' ); ?> role="article" itemscope itemtype="http://schema.org/BlogPosting">
								...
								<header class="article-header">
									...
									<p class="byline vcard">
										<?php printf( __( 'Posted <time class="updated" datetime="%1$s" itemprop="datePublished">%2$s</time> by <span class="author">%3$s</span>', 'skeletontheme' ), get_the_time('Y-m-j'), get_the_time(get_option('date_format')), get_the_author_link( get_the_author_meta( 'ID' ) )); ?>
									</p>
                  ...
                  <?php the_tags( '<p class="tags"><span class="tags-title">' . __( 'Tags:', 'skeletontheme' ) . '</span> ', ', ', '</p>' ); ?>
							...
							<?php endwhile; else : ?>
									...
									<article id="post-not-found" class="hentry cf">
										...
										<footer class="article-footer">
												<p><?php _e( 'This is the error message in the page-custom.php template.', 'skeletontheme' ); ?></p>
										</footer>
									</article>
